...
const ROUTES_DIR = path.join(__dirname, '../src/routers');
...
function getBasePaths(): Record<string, BasePathInfo> {
  const indexFile = path.join(ROUTES_DIR, 'index.ts');
  const content = fs.readFileSync(indexFile, 'utf-8');

  const basePaths: Record<string, BasePathInfo> = {};
  const lines = content.split('\n');
  let isProtected = false;

  for (const line of lines) {
    if (line.includes('router.use(verifyToken)')) {
      isProtected = true;
    }
    const match = line.match(/router\.use\(["']([^"']+)["']\s*,\s*(\w+)Router\)/);
    if (match) {
      basePaths[match[2]] = { path: match[1], isProtected };
    }
  }
...
async function main() {
  ...
  for (const [routerName, info] of Object.entries(basePaths)) {
    const basePath = info.path;
    const isGlobalProtected = info.isProtected;
    ...
    const hasMiddlewareAuth = routes.some(r => r.middleware.some(m =>
      m.toLowerCase().includes('jwt') ||
      m.toLowerCase().includes('rolecheck') ||
      m.toLowerCase().includes('verifytoken')
    ));
    const hasJwt = isGlobalProtected || hasMiddlewareAuth;
    ...
    for (const route of routes) {
      ...
      if (!schemaName && fs.existsSync(validationFile)) {
        try {
          ...
          for (const key of Object.keys(mod)) {
            ...
            if (keyLow === `${ctrlClean}schema` || keyLow === `${pathEnd}schema` ||
              (pathEnd === 'verifyotp' && keyLow === 'verifyotpschema')) {
              schemaName = key;
              break;
            }
          }
      ...
      const pathParams: string[] = fullPath.match(/:(\w+)/g) || [];
